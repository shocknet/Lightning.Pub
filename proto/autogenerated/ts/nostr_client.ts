...
export default (params: NostrClientParams,  send: (to:string, message: NostrRequest) => Promise<any>, subscribe: (to:string, message: NostrRequest, cb:(res:any)=> void) => void) => ({
    ...
    SubToUserWebRtcCandidates: async (cb: (res:ResultError | ({ status: 'OK' }& Types.WebRtcCandidate)) => void): Promise<void> => {
        const auth = await params.retrieveNostrUserAuth()
        if (auth === null) throw new Error('retrieveNostrUserAuth() returned null')
        const nostrRequest: NostrRequest = {}
        subscribe(params.pubDestination, {rpcName:'SubToUserWebRtcCandidates',authIdentifier:auth, ...nostrRequest }, (data) => {
            if (data.status === 'ERROR' && typeof data.reason === 'string') return cb(data)
            if (data.status === 'OK') { 
                const result = data
                if(!params.checkResult) return cb({ status: 'OK', ...result })
    ...
    SubmitUserWebRtcMessage: async (request: Types.WebRtcMessage): Promise<ResultError | ({ status: 'OK' }& Types.WebRtcAnswer)> => {
        const auth = await params.retrieveNostrUserAuth()
        if (auth === null) throw new Error('retrieveNostrUserAuth() returned null')
        const nostrRequest: NostrRequest = {}
        nostrRequest.body = request
        const data = await send(params.pubDestination, {rpcName:'SubmitUserWebRtcMessage',authIdentifier:auth, ...nostrRequest }) 
        if (data.status === 'ERROR' && typeof data.reason === 'string') return data
        if (data.status === 'OK') { 
            const result = data
            if(!params.checkResult) return { status: 'OK', ...result }
