    ...
    return async (req: NostrRequest, res: NostrResponse, startString: string, startMs: number) => {
        ...
        switch (req.rpcName) {
            ...
            case 'GetAdminNodeSettings':
                ...
                break
            case 'GetAdminOnchainConfSettings':
                try {
                    ...
                    authCtx = authContext
                    stats.validate = stats.guard
                    const response = await methods.GetAdminOnchainConfSettings({rpcName:'GetAdminOnchainConfSettings', ctx:authContext })
                    stats.handle = process.hrtime.bigint()
                    res({status: 'OK', ...response})
                    opts.metricsCallback([{ ...info, ...stats, ...authContext }])
                }catch(ex){ const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
            ...
            case 'ListPeers':
                try {
                    ...
                    authCtx = authContext
                    stats.validate = stats.guard
                    const response = await methods.ListPeers({rpcName:'ListPeers', ctx:authContext })
                    stats.handle = process.hrtime.bigint()
                    res({status: 'OK', ...response})
                    opts.metricsCallback([{ ...info, ...stats, ...authContext }])
                }catch(ex){ const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
            ...
            case 'ListUtxos':
                try {
                    ...
                    authCtx = authContext
                    stats.validate = stats.guard
                    const response = await methods.ListUtxos({rpcName:'ListUtxos', ctx:authContext })
                    stats.handle = process.hrtime.bigint()
                    res({status: 'OK', ...response})
                    opts.metricsCallback([{ ...info, ...stats, ...authContext }])
                }catch(ex){ const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
            ...
            case 'UpdateAdminNodeSettings':
                ...
                break
            case 'UpdateAdminOnchainConfSettings':
                try {
                    ...
                    stats.validate = process.hrtime.bigint()
                    if (error !== null) return logErrorAndReturnResponse(error, 'invalid request body', res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback)
                    const response = await methods.UpdateAdminOnchainConfSettings({rpcName:'UpdateAdminOnchainConfSettings', ctx:authContext , req: request})
                    stats.handle = process.hrtime.bigint()
                    res({status: 'OK', ...response})
                    opts.metricsCallback([{ ...info, ...stats, ...authContext }])
                }catch(ex){ const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
