    ...
    return async (req: NostrRequest, res: NostrResponse, startString: string, startMs: number) => {
        ...
        switch (req.rpcName) {
                            ...
                            switch(operation.rpcName) {
                                ...
                                case 'SubmitUserWebRtcMessage':
                                    if (!methods.SubmitUserWebRtcMessage) {
                                        throw new Error('method not defined: SubmitUserWebRtcMessage')
                                    } else {
            ...
            case 'RespondToDebit':
                ...
                break
            case 'SubToUserWebRtcCandidates':
                try {
                    if (!methods.SubToUserWebRtcCandidates) throw new Error('method: SubToUserWebRtcCandidates is not implemented')
                    const authContext = await opts.NostrUserAuthGuard(req.appId, req.authIdentifier)
                    stats.guard = process.hrtime.bigint()
                    authCtx = authContext
                    stats.validate = stats.guard
                    methods.SubToUserWebRtcCandidates({rpcName:'SubToUserWebRtcCandidates', ctx:authContext  ,cb: (response, err) => {
                    stats.handle = process.hrtime.bigint()
                    if (err) { logErrorAndReturnResponse(err, err.message, res, logger, { ...info, ...stats, ...authContext }, opts.metricsCallback)} else { res({status: 'OK', ...response});opts.metricsCallback([{ ...info, ...stats, ...authContext }])}
                    }})
                }catch(ex){ const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
