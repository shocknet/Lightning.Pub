...
export default (methods: Types.ServerMethods, opts: ServerOptions) => {
    ...
    if (!opts.allowNotImplementedMethods && !methods.SendAppUserToTwoAppUsersPayment) throw new Error('method: SendAppUserToTwoAppUsersPayment is not implemented')
    app.post('/api/app/multuser/internal/pay', async (req, res) => {
        try {
            if (!methods.SendAppUserToTwoAppUsersPayment) throw new Error('method: SendAppUserToTwoAppUsersPayment is not implemented')
            const authContext = await opts.AppAuthGuard(req.headers['authorization'])
            const request = req.body
            const error = Types.SendAppUserToTwoAppUsersPaymentRequestValidate(request)
            if (error !== null) return logErrorAndReturnResponse(error, 'invalid request body', res, logger)
            const query = req.query
            const params = req.params
            await methods.SendAppUserToTwoAppUsersPayment({ ...authContext, ...query, ...params }, request)
            if (overrides.SendAppUserToTwoAppUsersPayment_Override) await overrides.SendAppUserToTwoAppUsersPayment_Override(res); else res.json({status: 'OK'}) 
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger); if (opts.throwErrors) throw e }
    })
...
export type MethodsOverride = {
    ...
    SendAppUserToTwoAppUsersPayment_Override?: (httpRes:Response) => Promise<void>
