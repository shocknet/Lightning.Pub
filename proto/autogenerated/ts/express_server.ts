...
export type ServerOptions = {
    ...
    AdminAuthGuard: (authorizationHeader: string | undefined, requestContext: Types.RequestContext) => Promise<Types.AdminContext>
    AppAuthGuard: (authorizationHeader: string | undefined, requestContext: Types.RequestContext) => Promise<Types.AppContext>
    GuestAuthGuard: (authorizationHeader: string | undefined, requestContext: Types.RequestContext) => Promise<Types.GuestContext>
    GuestWithPubAuthGuard: (authorizationHeader: string | undefined, requestContext: Types.RequestContext) => Promise<Types.GuestWithPubContext>
    MetricsAuthGuard: (authorizationHeader: string | undefined, requestContext: Types.RequestContext) => Promise<Types.MetricsContext>
    UserAuthGuard: (authorizationHeader: string | undefined, requestContext: Types.RequestContext) => Promise<Types.UserContext>
...
const parseBody = (req: Request) => new Promise<Buffer>((resolve, reject) => {
    const chunks: Buffer[] = []
    req.on('data', (chunk) => {
        chunks.push(chunk)
    })
...
const createRequestContext = (req: Request, res?: Response): Types.RequestContext => {
    ...
    setHeader: (name: string, value: string) => {
        if (!res) throw new Error('setHeader is not available in this context')
        res.setHeader(name, value)
    },
    clearCookie: (name: string) => {
        if (!res) throw new Error('clearCookie is not available in this context')
        res.clearCookie(name)
    },
    getRequestLogger: () => requestLogger,
    setRequestLogger: (log: Types.RequestLogger) => { requestLogger = log },
    }
}
export default (methods: Types.ServerMethods, opts: ServerOptions) => {
    ...
    if (opts.configureApp) opts.configureApp(app)
    if (!opts.allowNotImplementedMethods && !methods.AddApp) throw new Error('method: AddApp is not implemented')
    app.post('/api/admin/app/add', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const response =  await methods.AddApp({rpcName:'AddApp', ctx:authContext , req: request, requestContext})
    ...
    app.post('/api/app/add/invoice', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const authContext = await opts.AppAuthGuard(req.headers['authorization'], requestContext)
            authCtx = authContext
            stats.guard = process.hrtime.bigint()
            const request = req.body
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.AddAppInvoice({rpcName:'AddAppInvoice', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/user/add', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.AddAppUser({rpcName:'AddAppUser', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/user/add/invoice', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.AddAppUserInvoice({rpcName:'AddAppUserInvoice', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/peer', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.AddPeer({rpcName:'AddPeer', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/product/add', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const authContext = await opts.UserAuthGuard(req.headers['authorization'], requestContext)
            authCtx = authContext
            stats.guard = process.hrtime.bigint()
            const request = req.body
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.AddProduct({rpcName:'AddProduct', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/offer/add', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.AddUserOffer({rpcName:'AddUserOffer', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/app/auth', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.AuthApp({rpcName:'AuthApp', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/manage/authorize', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.AuthorizeManage({rpcName:'AuthorizeManage', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/debit/ban', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.BanDebit({rpcName:'BanDebit', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/user/ban', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.BanUser({rpcName:'BanUser', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    })
    app.post('/api/user/batch', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const requestContext = createRequestContext(req, res)
            const ctx = await opts.UserAuthGuard(req.headers['authorization'], requestContext)
            stats.guard = process.hrtime.bigint()
                    ...
                    switch(operation.rpcName) {
                        case 'AddProduct':
                            if (!methods.AddProduct) {
                                ...
                                const rpcOut = await methods.AddProduct({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                        ...
                        case 'AddUserOffer':
                            if (!methods.AddUserOffer) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.AddUserOffer({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'AuthorizeManage':
                            if (!methods.AuthorizeManage) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.AuthorizeManage({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'BanDebit':
                            if (!methods.BanDebit) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.BanDebit({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'DecodeInvoice':
                            if (!methods.DecodeInvoice) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.DecodeInvoice({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'DeleteUserOffer':
                            if (!methods.DeleteUserOffer) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.DeleteUserOffer({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'EditDebit':
                            if (!methods.EditDebit) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.EditDebit({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'EnrollAdminToken':
                            if (!methods.EnrollAdminToken) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.EnrollAdminToken({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'EnrollMessagingToken':
                            if (!methods.EnrollMessagingToken) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.EnrollMessagingToken({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetDebitAuthorizations':
                            if (!methods.GetDebitAuthorizations) {
                                ...
                                const rpcOut = await methods.GetDebitAuthorizations({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetHttpCreds':
                            if (!methods.GetHttpCreds) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.GetHttpCreds({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetLNURLChannelLink':
                            if (!methods.GetLNURLChannelLink) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.GetLNURLChannelLink({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetLnurlPayLink':
                            if (!methods.GetLnurlPayLink) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.GetLnurlPayLink({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetLnurlWithdrawLink':
                            if (!methods.GetLnurlWithdrawLink) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.GetLnurlWithdrawLink({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetManageAuthorizations':
                            if (!methods.GetManageAuthorizations) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.GetManageAuthorizations({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetPaymentState':
                            if (!methods.GetPaymentState) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.GetPaymentState({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetTransactionSwapQuotes':
                            if (!methods.GetTransactionSwapQuotes) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.GetTransactionSwapQuotes({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetUserInfo':
                            if (!methods.GetUserInfo) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.GetUserInfo({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetUserOffer':
                            if (!methods.GetUserOffer) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.GetUserOffer({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetUserOfferInvoices':
                            if (!methods.GetUserOfferInvoices) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.GetUserOfferInvoices({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetUserOffers':
                            if (!methods.GetUserOffers) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.GetUserOffers({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'GetUserOperations':
                            if (!methods.GetUserOperations) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.GetUserOperations({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'ListTxSwaps':
                            if (!methods.ListTxSwaps) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.ListTxSwaps({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'NewAddress':
                            if (!methods.NewAddress) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.NewAddress({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'NewInvoice':
                            if (!methods.NewInvoice) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.NewInvoice({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'NewProductInvoice':
                            if (!methods.NewProductInvoice) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.NewProductInvoice({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'PayAddress':
                            if (!methods.PayAddress) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.PayAddress({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'PayInvoice':
                            if (!methods.PayInvoice) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.PayInvoice({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'ResetDebit':
                            if (!methods.ResetDebit) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.ResetDebit({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'ResetManage':
                            if (!methods.ResetManage) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.ResetManage({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'RespondToDebit':
                            if (!methods.RespondToDebit) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.RespondToDebit({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'UpdateCallbackUrl':
                            if (!methods.UpdateCallbackUrl) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                const rpcOut = await methods.UpdateCallbackUrl({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'UpdateUserOffer':
                            if (!methods.UpdateUserOffer) {
                                ...
                                opStats.validate = process.hrtime.bigint()
                                if (error !== null) throw error
                                await methods.UpdateUserOffer({...operation, ctx, requestContext}); responses.push({ status: 'OK' })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
                        case 'UserHealth':
                            if (!methods.UserHealth) {
                                ...
                                opStats.validate = opStats.guard
                                const rpcOut = await methods.UserHealth({...operation, ctx, requestContext}); responses.push({ status: 'OK', ...rpcOut  })
                                opStats.handle = process.hrtime.bigint()
                                callsMetrics.push({ ...opInfo, ...opStats, ...ctx })
                            }
                            break
    ...
    app.post('/api/admin/tx/bump', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.BumpTx({rpcName:'BumpTx', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/channel/close', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.CloseChannel({rpcName:'CloseChannel', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/app/invite/create', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.CreateOneTimeInviteLink({rpcName:'CreateOneTimeInviteLink', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/invoice/decode', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.DecodeInvoice({rpcName:'DecodeInvoice', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/offer/delete', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.DeleteUserOffer({rpcName:'DeleteUserOffer', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/debit/edit', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.EditDebit({rpcName:'EditDebit', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/encryption/exchange', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const authContext = await opts.GuestAuthGuard(req.headers['authorization'], requestContext)
            authCtx = authContext
            stats.guard = process.hrtime.bigint()
            const request = req.body
            ...
            const query = req.query
            const params = req.params
             await methods.EncryptionExchange({rpcName:'EncryptionExchange', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/guest/npub/enroll/admin', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.EnrollAdminToken({rpcName:'EnrollAdminToken', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/messaging/enroll', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.EnrollMessagingToken({rpcName:'EnrollMessagingToken', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/swap/invoice/quote', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetAdminInvoiceSwapQuotes({rpcName:'GetAdminInvoiceSwapQuotes', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/swap/transaction/quote', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetAdminTransactionSwapQuotes({rpcName:'GetAdminTransactionSwapQuotes', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/get', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetApp({rpcName:'GetApp', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/user/get', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetAppUser({rpcName:'GetAppUser', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/user/lnurl/pay/info', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetAppUserLNURLInfo({rpcName:'GetAppUserLNURLInfo', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/reports/apps', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const authContext = await opts.MetricsAuthGuard(req.headers['authorization'], requestContext)
            authCtx = authContext
            stats.guard = process.hrtime.bigint()
            const request = req.body
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetAppsMetrics({rpcName:'GetAppsMetrics', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/assets/liabilities', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetAssetsAndLiabilities({rpcName:'GetAssetsAndLiabilities', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    })
    if (!opts.allowNotImplementedMethods && !methods.GetAssetsAndLiabilitiesV2) throw new Error('method: GetAssetsAndLiabilitiesV2 is not implemented')
    app.post('/api/admin/assets/liabilities/v2', json(), urlencoded({ extended: true }), async (req, res) => {
        const info: Types.RequestInfo = { rpcName: 'GetAssetsAndLiabilitiesV2', batch: false, nostr: false, batchSize: 0}
        const stats: Types.RequestStats = { startMs:req.startTimeMs || 0, start:req.startTime || 0n, parse: process.hrtime.bigint(), guard: 0n, validate: 0n, handle: 0n }
        let authCtx: Types.AuthContext = {}
        const requestContext = createRequestContext(req, res)
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetAssetsAndLiabilitiesV2({rpcName:'GetAssetsAndLiabilitiesV2', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/reports/bundle', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetBundleMetrics({rpcName:'GetBundleMetrics', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/user/debit/get', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetDebitAuthorizations({rpcName:'GetDebitAuthorizations', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/reports/errors', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetErrorStats({rpcName:'GetErrorStats', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/http_creds', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetHttpCreds({rpcName:'GetHttpCreds', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/app/invite/get', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetInviteLinkState({rpcName:'GetInviteLinkState', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/lnurl_channel/url', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetLNURLChannelLink({rpcName:'GetLNURLChannelLink', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    })
    if (!opts.allowNotImplementedMethods && !methods.GetLiveDebitRequests) throw new Error('method: GetLiveDebitRequests is not implemented')
    app.ws('/api/user/debit/sub', async (ws, req) => {
        let authorized = false
        let more: ((bytes: Object | Buffer) => void) | undefined = undefined
        ws.on('message', async (msg) => {
            try {
                if (!authorized) {
                    ...
                    const authContext = await opts.UserAuthGuard(j._authorization, requestContext)
                    delete j._authorization
                    authorized = true
                    const query = req.query
                    const params = req.params
                    methods.GetLiveDebitRequests({rpcName:'GetLiveDebitRequests', ctx:authContext , requestContext, socket:{ send: (response, err, cb) => {
                    if (err) { logger.error(err.message || err); ws.send(JSON.stringify({ status: 'ERROR', reason: err.message || err })) }
                    else { ws.send(JSON.stringify({status: 'OK', ...response}), cb) }
                    },subMore: cb=>more=cb, getState: () => [0, 1].includes(ws.readyState) ? 'OPEN' : 'CLOSED', close: () => ws.close() }})
                } else if (more) more(msg.valueOf())
            } catch(error: any) {logger.error(error.message || error); ws.send(JSON.stringify({ status: 'ERROR', reason: error.message || error })) }
        })
    })
    if (!opts.allowNotImplementedMethods && !methods.GetLiveManageRequests) throw new Error('method: GetLiveManageRequests is not implemented')
    app.ws('/api/user/manage/sub', async (ws, req) => {
        let authorized = false
        let more: ((bytes: Object | Buffer) => void) | undefined = undefined
        ws.on('message', async (msg) => {
            try {
                if (!authorized) {
                    ...
                    const query = req.query
                    const params = req.params
                    methods.GetLiveManageRequests({rpcName:'GetLiveManageRequests', ctx:authContext , requestContext, socket:{ send: (response, err, cb) => {
                    if (err) { logger.error(err.message || err); ws.send(JSON.stringify({ status: 'ERROR', reason: err.message || err })) }
                    else { ws.send(JSON.stringify({status: 'OK', ...response}), cb) }
                    },subMore: cb=>more=cb, getState: () => [0, 1].includes(ws.readyState) ? 'OPEN' : 'CLOSED', close: () => ws.close() }})
                } else if (more) more(msg.valueOf())
    ...
    if (!opts.allowNotImplementedMethods && !methods.GetLiveUserOperations) throw new Error('method: GetLiveUserOperations is not implemented')
    app.ws('/api/user/operations/sub', async (ws, req) => {
        let authorized = false
        let more: ((bytes: Object | Buffer) => void) | undefined = undefined
        ws.on('message', async (msg) => {
            try {
                if (!authorized) {
                    ...
                    const query = req.query
                    const params = req.params
                    methods.GetLiveUserOperations({rpcName:'GetLiveUserOperations', ctx:authContext , requestContext, socket:{ send: (response, err, cb) => {
                    if (err) { logger.error(err.message || err); ws.send(JSON.stringify({ status: 'ERROR', reason: err.message || err })) }
                    else { ws.send(JSON.stringify({status: 'OK', ...response}), cb) }
                    },subMore: cb=>more=cb, getState: () => [0, 1].includes(ws.readyState) ? 'OPEN' : 'CLOSED', close: () => ws.close() }})
                } else if (more) more(msg.valueOf())
    ...
    app.post('/api/reports/lnd/forwarding', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetLndForwardingMetrics({rpcName:'GetLndForwardingMetrics', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/reports/lnd', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetLndMetrics({rpcName:'GetLndMetrics', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/guest/lnurl_pay/info', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetLnurlPayInfo({rpcName:'GetLnurlPayInfo', ctx:authContext ,query: req.query, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/user/lnurl_pay/link', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetLnurlPayLink({rpcName:'GetLnurlPayLink', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/guest/lnurl_withdraw/info', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetLnurlWithdrawInfo({rpcName:'GetLnurlWithdrawInfo', ctx:authContext ,query: req.query, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/user/lnurl_withdraw/link', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetLnurlWithdrawLink({rpcName:'GetLnurlWithdrawLink', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/user/manage/get', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetManageAuthorizations({rpcName:'GetManageAuthorizations', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    })
    if (!opts.allowNotImplementedMethods && !methods.GetMigrationUpdate) throw new Error('method: GetMigrationUpdate is not implemented')
    app.ws('/api/user/migrations/sub', async (ws, req) => {
        let authorized = false
        let more: ((bytes: Object | Buffer) => void) | undefined = undefined
        ws.on('message', async (msg) => {
            try {
                if (!authorized) {
                    ...
                    const query = req.query
                    const params = req.params
                    methods.GetMigrationUpdate({rpcName:'GetMigrationUpdate', ctx:authContext , requestContext, socket:{ send: (response, err, cb) => {
                    if (err) { logger.error(err.message || err); ws.send(JSON.stringify({ status: 'ERROR', reason: err.message || err })) }
                    else { ws.send(JSON.stringify({status: 'OK', ...response}), cb) }
                    },subMore: cb=>more=cb, getState: () => [0, 1].includes(ws.readyState) ? 'OPEN' : 'CLOSED', close: () => ws.close() }})
                } else if (more) more(msg.valueOf())
    ...
    app.post('/api/app/user/npub/state', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetNPubLinkingState({rpcName:'GetNPubLinkingState', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/payment/state', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetPaymentState({rpcName:'GetPaymentState', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/metrics/providers/disruption', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetProvidersDisruption({rpcName:'GetProvidersDisruption', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/admin/seed', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetSeed({rpcName:'GetSeed', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/reports/bundle/single', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetSingleBundleMetrics({rpcName:'GetSingleBundleMetrics', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/reports/usage/single', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetSingleUsageMetrics({rpcName:'GetSingleUsageMetrics', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/swap/transaction/quote', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetTransactionSwapQuotes({rpcName:'GetTransactionSwapQuotes', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/reports/usage', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUsageMetrics({rpcName:'GetUsageMetrics', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/info', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUserInfo({rpcName:'GetUserInfo', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/offer/get', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUserOffer({rpcName:'GetUserOffer', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/offer/get/invoices', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUserOfferInvoices({rpcName:'GetUserOfferInvoices', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/user/offers/get', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUserOffers({rpcName:'GetUserOffers', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/operations', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUserOperations({rpcName:'GetUserOperations', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    })
    if (!opts.allowNotImplementedMethods && !methods.GetUserOperationsFromAdmin) throw new Error('method: GetUserOperationsFromAdmin is not implemented')
    app.post('/api/admin/user/operations', json(), urlencoded({ extended: true }), async (req, res) => {
        const info: Types.RequestInfo = { rpcName: 'GetUserOperationsFromAdmin', batch: false, nostr: false, batchSize: 0}
        const stats: Types.RequestStats = { startMs:req.startTimeMs || 0, start:req.startTime || 0n, parse: process.hrtime.bigint(), guard: 0n, validate: 0n, handle: 0n }
        let authCtx: Types.AuthContext = {}
        const requestContext = createRequestContext(req, res)
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUserOperationsFromAdmin({rpcName:'GetUserOperationsFromAdmin', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    })
    if (!opts.allowNotImplementedMethods && !methods.GetUsersAdminInfo) throw new Error('method: GetUsersAdminInfo is not implemented')
    app.post('/api/admin/users/info', json(), urlencoded({ extended: true }), async (req, res) => {
        const info: Types.RequestInfo = { rpcName: 'GetUsersAdminInfo', batch: false, nostr: false, batchSize: 0}
        const stats: Types.RequestStats = { startMs:req.startTimeMs || 0, start:req.startTime || 0n, parse: process.hrtime.bigint(), guard: 0n, validate: 0n, handle: 0n }
        let authCtx: Types.AuthContext = {}
        const requestContext = createRequestContext(req, res)
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.GetUsersAdminInfo({rpcName:'GetUsersAdminInfo', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/.well-known/lnurlp/:address_name', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.HandleLnurlAddress({rpcName:'HandleLnurlAddress', ctx:authContext ,params: req.params, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/guest/lnurl_pay/handle', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.HandleLnurlPay({rpcName:'HandleLnurlPay', ctx:authContext ,query: req.query, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/guest/lnurl_withdraw/handle', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.HandleLnurlWithdraw({rpcName:'HandleLnurlWithdraw', ctx:authContext ,query: req.query, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/health', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.Health({rpcName:'Health', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/guest/npub/link', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const authContext = await opts.GuestWithPubAuthGuard(req.headers['authorization'], requestContext)
            authCtx = authContext
            stats.guard = process.hrtime.bigint()
            const request = req.body
            ...
            const query = req.query
            const params = req.params
             await methods.LinkNPubThroughToken({rpcName:'LinkNPubThroughToken', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/swap/invoice/list', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.ListAdminInvoiceSwaps({rpcName:'ListAdminInvoiceSwaps', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/swap/transaction/list', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.ListAdminTxSwaps({rpcName:'ListAdminTxSwaps', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/admin/channels', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.ListChannels({rpcName:'ListChannels', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/lnd/getinfo', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.LndGetInfo({rpcName:'LndGetInfo', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/chain/new', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.NewAddress({rpcName:'NewAddress', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/invoice/new', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.NewInvoice({rpcName:'NewInvoice', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.get('/api/user/product/get/invoice', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.NewProductInvoice({rpcName:'NewProductInvoice', ctx:authContext ,query: req.query, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/channel/open', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.OpenChannel({rpcName:'OpenChannel', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/chain/pay', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.PayAddress({rpcName:'PayAddress', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/swap/invoice/pay', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.PayAdminInvoiceSwap({rpcName:'PayAdminInvoiceSwap', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/swap/transaction/pay', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.PayAdminTransactionSwap({rpcName:'PayAdminTransactionSwap', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/invoice/pay', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.PayAppUserInvoice({rpcName:'PayAppUserInvoice', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/invoice/pay', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.PayInvoice({rpcName:'PayInvoice', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/metrics/ping', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.PingSubProcesses({rpcName:'PingSubProcesses', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/swap/invoice/refund', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.RefundAdminInvoiceSwap({rpcName:'RefundAdminInvoiceSwap', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/user/npub/token', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.RequestNPubLinkingToken({rpcName:'RequestNPubLinkingToken', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/debit/reset', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.ResetDebit({rpcName:'ResetDebit', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/manage/reset', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.ResetManage({rpcName:'ResetManage', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/metrics/reset', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.ResetMetricsStorages({rpcName:'ResetMetricsStorages', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/user/npub/token/reset', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.ResetNPubLinkingToken({rpcName:'ResetNPubLinkingToken', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/debit/finish', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.RespondToDebit({rpcName:'RespondToDebit', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/internal/pay', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.SendAppUserToAppPayment({rpcName:'SendAppUserToAppPayment', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/user/internal/pay', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.SendAppUserToAppUserPayment({rpcName:'SendAppUserToAppUserPayment', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/mock/blance/set', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.SetMockAppBalance({rpcName:'SetMockAppBalance', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/app/mock/user/blance/set', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.SetMockAppUserBalance({rpcName:'SetMockAppUserBalance', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/lnd/mock/invoice/paid', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.SetMockInvoiceAsPaid({rpcName:'SetMockInvoiceAsPaid', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    })
    if (!opts.allowNotImplementedMethods && !methods.SubToWebRtcCandidates) throw new Error('method: SubToWebRtcCandidates is not implemented')
    app.ws('/api/upgrade/wrtc/candidates', async (ws, req) => {
        let authorized = false
        let more: ((bytes: Object | Buffer) => void) | undefined = undefined
        ws.on('message', async (msg) => {
            try {
                if (!authorized) {
                    ...
                    const j = JSON.parse(msg.toString())
                    const requestContext = createRequestContext(req)
                    const authContext = await opts.MetricsAuthGuard(j._authorization, requestContext)
                    delete j._authorization
                    authorized = true
                    const query = req.query
                    const params = req.params
                    methods.SubToWebRtcCandidates({rpcName:'SubToWebRtcCandidates', ctx:authContext , requestContext, socket:{ send: (response, err, cb) => {
                    if (err) { logger.error(err.message || err); ws.send(JSON.stringify({ status: 'ERROR', reason: err.message || err })) }
                    else { ws.send(JSON.stringify({status: 'OK', ...response}), cb) }
                    },subMore: cb=>more=cb, getState: () => [0, 1].includes(ws.readyState) ? 'OPEN' : 'CLOSED', close: () => ws.close() }})
                } else if (more) more(msg.valueOf())
    ...
    app.post('/api/upgrade/wrtc', json(), urlencoded({ extended: true }), async (req, res) => {
    ...
    app.post('/api/user/cb/update', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.UpdateCallbackUrl({rpcName:'UpdateCallbackUrl', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/admin/channel/policy/update', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.UpdateChannelPolicy({rpcName:'UpdateChannelPolicy', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/offer/update', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.UpdateUserOffer({rpcName:'UpdateUserOffer', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/guest/invite', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
             await methods.UseInviteLink({rpcName:'UseInviteLink', ctx:authContext , req: request, requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK'})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/user/health', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.UserHealth({rpcName:'UserHealth', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
    ...
    app.post('/api/metrics/zip', json(), urlencoded({ extended: true }), async (req, res) => {
        ...
        try {
            ...
            const query = req.query
            const params = req.params
            const response =  await methods.ZipMetricsStorages({rpcName:'ZipMetricsStorages', ctx:authContext , requestContext})
            stats.handle = process.hrtime.bigint()
            res.json({status: 'OK', ...response})
            opts.metricsCallback([{ ...info, ...stats, ...authContext }])
        } catch (ex) { const e = ex as any; logErrorAndReturnResponse(e, e.message || e, res, logger, { ...info, ...stats, ...authCtx }, opts.metricsCallback); if (opts.throwErrors) throw e }
