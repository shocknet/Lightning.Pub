...
import { NdebitData, NmanageRequest } from "@shocknet/clink-sdk";
import { CLINK_ENROLL_KIND, CLINK_OFFER_KIND } from "./services/CLINK/clinkConstants.js";
import { ClinkMethods } from "./services/serverMethods/clinkMethods.js";
...
export default (
    ...
    const clinkTransport = newClinkTransport(clinkMethods )

    const nostr = new Nostr(nostrSettings, mainHandler.utils, event => {
        if (event.kind === CLINK_OFFER_KIND) {
            ...
            void clinkTransport(event, reply => {
                ...
                log("📤 [OFFER RESPONSE] Sent offer response", {
                    toPub: event.pub,
                    eventId: event.id,
                })
            }).catch(err => log(ERROR, "clink Offer failed", err.message || err))
            return
        }
