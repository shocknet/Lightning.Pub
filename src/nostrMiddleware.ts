...
export default (serverMethods: Types.ServerMethods, mainHandler: Main, nostrSettings: NostrSettings, onClientEvent: ClientEventCallback): ExportedCalls => {
    ...
    const nostr = new Nostr(nostrSettings, mainHandler.utils, event => {
        ...
        try {
            ...
            if (event.kind === 21002) {
                mainHandler.debitManager.sendDebitResponse(ndebitInvalidRequest("malformed request"), event)
            }
            return
        }
        if (event.kind === 21001 || event.kind === 21002 || event.kind === 21003) {
            if (!isSupportedClinkEvent(event.tags)) {
                log("dropping unsupported clink_version", event.kind, event.id)
                if (event.kind === 21002) {
                    mainHandler.debitManager.sendDebitResponse(ndebitInvalidRequest("unsupported clink_version"), event)
                }
                return
            }
        }
        if (event.kind === 21001) {
            ...
            try {
                validateNdebitData(debitReq)
            } catch (err: any) {
                mainHandler.debitManager.sendDebitResponse(ndebitInvalidRequest(err.message || "malformed request"), event)
                return
            }
            void mainHandler.debitManager.handleNdebit(debitReq, event).catch(err => log(ERROR, "handleNdebit failed", err.message || err))
            return
