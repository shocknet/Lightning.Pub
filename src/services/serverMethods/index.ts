...
import Main from '../main/index.js'
import WebRTC2 from '../webRTC/wrtc2.js'
export default (mainHandler: Main, wrtc2: WebRTC2): Types.ServerMethods => {
    return {
        ...
        SubmitUserWebRtcMessage: async ({ ctx, req }) => { // TODO: this is a hack for a test 
            const err = Types.WebRtcMessageValidate(req, {
                message_Options: {
                    candidate_CustomCheck: candidate => candidate !== '',
                    offer_CustomCheck: offer => offer !== '',
            ...
            if (err != null) throw new Error(err.message)
            const app = await mainHandler.storage.applicationStorage.GetApplication(ctx.app_id)
            const appUser = await mainHandler.storage.applicationStorage.GetApplicationUser(app, ctx.app_user_id)
            return wrtc2.OnMessage({ userPub: appUser.nostr_public_key!, appId: ctx.app_id, appUserId: ctx.app_user_id, userId: ctx.user_id }, req.message)
        },
        SubToUserWebRtcCandidates: async ({ ctx }) => { }, // TODO: this is a hack for a test
    }
}