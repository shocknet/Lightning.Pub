...
import { isValidLspThreshold, isValidNodeName, isValidOnchainConfSettings } from '../main/adminNodeSettings.js'

const isNonNegativeSats = (amount: number) => Number.isSafeInteger(amount) && amount >= 0
const isPositiveSats = (amount: number) => Number.isSafeInteger(amount) && amount > 0
const isPositiveConfs = (confs: number) => Number.isSafeInteger(confs) && confs >= 1
const isValidOnchainConfRequest = (req: Types.UpdateAdminOnchainConfSettingsRequest) =>
    isValidOnchainConfSettings({
        tier1LimitSats: req.tier1_limit_sats,
        tier1Confs: req.tier1_confs,
        tier2LimitSats: req.tier2_limit_sats,
        tier2Confs: req.tier2_confs,
        tier3Confs: req.tier3_confs,
    })
...
export default (mainHandler: Main): Types.ServerMethods => {
    return {
        ...
        ListUtxos: async ({ ctx }) => {
            return mainHandler.adminManager.ListUtxos()
        },
        ...
        OpenChannel: async ({ ctx, req }) => {
            const err = Types.OpenChannelRequestValidate(req, {
                ...
                local_funding_amount_CustomCheck: isPositiveSats,
        ...
        GetAdminTransactionSwapQuotes: async ({ ctx, req }) => {
            const err = Types.TransactionSwapRequestValidate(req, {
                transaction_amount_sats_CustomCheck: isPositiveSats
            })
            if (err != null) throw new Error(err.message)
        ...
        GetAdminInvoiceSwapQuotes: async ({ ctx, req }) => {
            const err = Types.InvoiceSwapRequestValidate(req, {
                amount_sats_CustomCheck: isPositiveSats,
                fees_req_Options: {
                    fees_Options: {
                        sat_per_v_byte_CustomCheck: spv => spv > 0,
        ...
        GetAdminNodeSettings: async ({ ctx }) => {
            return mainHandler.adminManager.GetAdminNodeSettings()
        },
        UpdateAdminNodeSettings: async ({ ctx, req }) => {
            const err = Types.UpdateAdminNodeSettingsRequestValidate(req, {
                node_name_CustomCheck: isValidNodeName,
                avatar_url_CustomCheck: isHttpsAvatarUrl,
                lsp_channel_threshold_CustomCheck: isValidLspThreshold,
            })
            if (err != null) throw new Error(err.message)
            return mainHandler.adminManager.UpdateAdminNodeSettings(req)
        },
        GetAdminOnchainConfSettings: async ({ ctx }) => {
            return mainHandler.adminManager.GetAdminOnchainConfSettings()
        },
        UpdateAdminOnchainConfSettings: async ({ ctx, req }) => {
            const err = Types.UpdateAdminOnchainConfSettingsRequestValidate(req, {
                tier1_limit_sats_CustomCheck: isNonNegativeSats,
                tier1_confs_CustomCheck: isPositiveConfs,
                tier2_limit_sats_CustomCheck: isNonNegativeSats,
                tier2_confs_CustomCheck: isPositiveConfs,
                tier3_confs_CustomCheck: isPositiveConfs,
            })
            if (err != null) throw new Error(err.message)
            if (!isValidOnchainConfRequest(req)) {
            ...
            return mainHandler.adminManager.UpdateAdminOnchainConfSettings(req)
        },
        GetSeed: async ({ ctx }) => {
        ...
        PayAddress: async ({ ctx, req }) => {
            const err = Types.PayAddressRequestValidate(req, {
                address_CustomCheck: addr => addr !== '',
                amountSats_CustomCheck: isNonNegativeSats,
        ...
        PayInvoice: async ({ ctx, req }) => {
            const err = Types.PayInvoiceRequestValidate(req, {
                invoice_CustomCheck: invoice => invoice !== '',
                amount_CustomCheck: isNonNegativeSats
            })
            if (err != null) throw new Error(err.message)
        ...
        SendAppUserToAppUserPayment: async ({ ctx, req }) => {
            const err = Types.SendAppUserToAppUserPaymentRequestValidate(req, {
                ...
                amount_CustomCheck: isPositiveSats
            })
            if (err != null) throw new Error(err.message)
