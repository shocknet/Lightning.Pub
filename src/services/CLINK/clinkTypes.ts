...
export class ClinkError extends Error {
    ...
    getPayload = () => {
        return { code: this.code, error: this.message, res: "GFY" }
    }
}

export type ClinkSend = (event: UnsignedEvent) => void

export type ClinkCtx = {
    pub: string
    appId: string
    eventId: string
    tags?: string[][]
...
export const encodeClinkResponse = (kind: number, body: unknown, event: { pub: string, id: string }): UnsignedEvent => ({
