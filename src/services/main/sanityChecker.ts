...
export default class SanityChecker {
    ...
    liveUsers: Record<string, boolean> = {}
    ...
    async VerifyEventsLog() {
        ...
        for (let i = 0; i < this.events.length; i++) {
            ...
            if (e.type === 'balance_decrement') {
                ...
                await this.verifyTrackedEvent(e)
            }
        }
    ...
    private async userIsLive(userId: string) {
        if (this.liveUsers[userId] === undefined) {
            const user = await this.storage.userStorage.FindUser(userId)
            this.liveUsers[userId] = !!user
        }
        return this.liveUsers[userId]
    }

    private async verifyTrackedEvent(e: LoggedEvent) {
        switch (e.type) {
            case 'new_invoice':
                return this.verifyNewInvoice(e)
            case 'new_address':
                return this.verifyNewAddress(e)
            default:
                return this.storage.paymentStorage.VerifyDbEvent(e)
        }
    }

    private async verifyNewInvoice(e: LoggedEvent) {
        const entry = await this.storage.paymentStorage.GetInvoiceOwner(e.data)
        if (!entry) {
            return
        }
        if (entry.user.user_id !== e.userId) {
            throw new Error("invoice user id mismatch for " + e.data)
        }
    }

    private async verifyNewAddress(e: LoggedEvent) {
        const entry = await this.storage.paymentStorage.GetAddressOwner(e.data)
        if (entry) {
            if (entry.user.user_id !== e.userId) {
                throw new Error("address user id mismatch for " + e.data)
            }
            return
        }
    ...
    private async verifyReconstructedBalances() {
        await Promise.all(Object.entries(this.users).map(async ([userId, u]) => {
            const user = await this.storage.userStorage.FindUser(userId)
            if (!user) {
                throw new Error("sanity check on balance failed, user " + userId + " was deleted but event log reconstructs " + u.updatedBalance + " sats")
            }
