...
export class Unlocker {
    ...
    private publishScbToNostr = async (scb: Buffer) => {
        ...
        if (!local?.nostr_private_key || !local.nostr_public_key) {
            this.log("skipping SCB relay backup publish: local app nostr keys unavailable")
            return
        }
