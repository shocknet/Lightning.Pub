...
import { LndNodeInfo } from '../services/storage/entity/LndNodeInfo.js'
...
export default async (T: TestBase) => {
    ...
    await expectSeedSavedUnderDerivedKey(T, pubkey)
    T.d("Funding wallet...")
...
const expectSeedSavedUnderDerivedKey = async (T: TestBase, pubkey: string) => {
    T.expect(T.main.unlocker.nodePub).to.equal(pubkey)
    const nodes = await T.main.storage.liquidityStorage.dbs.Find<LndNodeInfo>('LndNodeInfo', {})
    const seeded = nodes.filter(n => !!n.seed)
    T.expect(seeded.map(n => n.pubkey)).to.deep.equal([pubkey])
    // Fresh DB: the two derived-key rows are serial 1 and 2; a fallback row saved from lnd's key would be 3.
    T.expect(seeded[0].serial_id).to.be.at.most(2)
    T.d("seed saved under key derived from entropy, matching lnd: " + pubkey)
}
