...
import { k1AlreadyProcessedReason, nofferErrors, ndebitInvalidRequest, invoiceAlreadyPaidReason, invoiceAlreadyFailedReason, invoicePaymentInProgressReason } from "../services/main/debitTypes.js"
...
const testPayNdebitInvoiceK1Dedup = async (T: TestBase) => {
    ...
    expectDebitFail(T, second, 6, ndebitInvalidRequest(k1AlreadyProcessedReason).error)
...
const testPayNdebitInvoiceRetryAlreadyPaid = async (T: TestBase) => {
    ...
    expectDebitFail(T, second, 6, ndebitInvalidRequest(invoiceAlreadyPaidReason).error)
...
const testPayNdebitInvoiceRetryAlreadyFailed = async (T: TestBase) => {
    ...
    const result = await payDebitInvoice(T, npub, invoice.payRequest, 500, "already-failed")
    expectDebitFail(T, result, 6, ndebitInvalidRequest(invoiceAlreadyFailedReason).error)
    T.d("payNdebitInvoice returns GFY 6 when the invoice already failed")
}
...
const testPayNdebitInvoiceRetryInProgressLock = async (T: TestBase) => {
    ...
    try {
        ...
        expectDebitFail(T, result, 6, ndebitInvalidRequest(invoicePaymentInProgressReason).error)
    } finally {
