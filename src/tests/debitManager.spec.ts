...
import { expectThrowsAsync, runSanityCheck, safelySetUserBalance, setUserBalanceExact, TestBase } from "./testBase.js"
...
/** Same as the once-per-file user2 funding in export default; reused for late live-invoice cases. */
const USER2_LIVE_INVOICE_SUITE_BALANCE = 10_000
...
const testConcurrentInvoiceSameLivePaysOnce = async (T: TestBase) => {
    ...
    await setUserBalanceExact(T, T.user2, USER2_LIVE_INVOICE_SUITE_BALANCE)
    ...
    const sendCost = await debitSendCost(T, 500)
    const before = await userBalance(T)
    T.expect(before).to.equal(USER2_LIVE_INVOICE_SUITE_BALANCE)
    const results = await Promise.allSettled([
        T.main.debitManager.RespondToDebit(userContext(T, T.user2), {
            npub,
            request_id: requestId,
    ...
    T.expect(after).to.equal(USER2_LIVE_INVOICE_SUITE_BALANCE - sendCost)
    T.expect(before - after).to.equal(sendCost)
...
const testRepeatInvoiceAfterSuccessDoesNotDrain = async (T: TestBase) => {
    ...
    await setUserBalanceExact(T, T.user2, USER2_LIVE_INVOICE_SUITE_BALANCE)
    T.expect(await userBalance(T)).to.equal(USER2_LIVE_INVOICE_SUITE_BALANCE)
